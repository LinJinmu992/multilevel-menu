...
#include "MenuIP.h"
#include "MenuData.h"
#define OLED_Clear() MenuPort_Clear()
#define OLED_ShowChar MenuPort_ShowChar
#define OLED_ShowString MenuPort_ShowString
#define OLED_ShowNum MenuPort_ShowNum
...
uint16_t Getvalue (uint16_t Value)
{
	if((MenuIP_GetCursor(IP) == 2) && MenuPort_IsConfirmPressed())
		Value ++;
	if((MenuIP_GetCursor(IP) == 3) && MenuPort_IsConfirmPressed())
		Value --;
	if((MenuIP_GetCursor(IP) == 4) && MenuPort_IsConfirmPressed())
	{
